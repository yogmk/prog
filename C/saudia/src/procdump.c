...
*/
	...
	/*puts ("input filename required."); */
        puts ("usage : exe-name <INFILE> 'OBJECT' <OUTFILE>"); 
        puts ("where OBJECT is one of the following ");
    ...
    if (ac == 3)      
    {
	puts ("OUTFILE name not provided. Will write to stdout.");
	ostream = stdout;      
    }
...
*/
    ...
    free (object_structure); object_structure = NULL;
    return 0;
}
...
conv (uchar *buf, uchar *ascii_buff, const unsigned int len, FIELD * object_structure, unsigned int no_fields)
{
    ...
    {
      FIELD curr_field = object_structure[i];            
      ...
      switch(datatype)
      {
        ...
        case E_DECIMAL:
			...
			strcpy(temp, decimal_output);
                        free (decimal_input); decimal_input = NULL;
                        free (decimal_output); decimal_output = NULL;
			break;
	case E_ZONED:
                        ...
                        j += width;
                        strcpy(temp, char_output);
                        free (char_input); char_input = NULL;
                        free (char_output); char_output = NULL;
                        break;
      }
