...
void conv (uchar*, uchar *, const unsigned int);
...
main (int ac, char** av)
{
    recLength = L_OBJ22OPN;    /* Customer master*/   /* TODO */

    uchar buf [recLength+1];
    uchar ascii_buf[recLength +50];
    int ret = 0;
    unsigned int j=0;
    ...
    else if(NULL == (ostream = fopen (av[2], "w")))
    {
	perror (av[2]);
	exit (EXIT_FAILURE);
    }
    ...
    while ((ret = readRec(buf, istream, (unsigned int)recLength)) > 0)
    {
       ...
       conv (buf, ascii_buf, (unsigned int)recLength); 
...
decimal2char(uchar *input, short width, short precision, uchar *fin_output)
{
 ...
 for (i=0; i<(width+1)/2 ; i++)
 {
   ...
   if (precision > 0)         /** TODO:if the number does not have a fraction, why bother ? */
   {
       if(i == (width-precision+1)/2 -1)
       {
         output[j] = '.';
         j++;
       }
   ...
   if( i <  (width+1)/2 - 1)   /** what is the purpose of this check? */
   {
     nibble = byte & 0x0f;
     nibble+=48;
     output[j]= nibble;
...
void
conv (uchar *buf, uchar *ascii_buff, const unsigned int len)
{
    unsigned int c = 0;
    short s;
    int intgr;
    unsigned int i, j = 0,k=0;
    unsigned int no_fields = sizeof(struOBJ22OPN)/ sizeof(FIELD);            /** TODO: change as per the object*/

    int index =0;

    for (i=0; i<no_fields; i++)
    {
      FIELD curr_field = struOBJ22OPN[i];            /** TODO: */
        ...
        case E_DECIMAL:
			...
			for(k=0; k<(width+1)/2; k++)
			  decimal_input[k] = buf[j+k];
                        decimal_input[k] = '\0';                /** TODO: shyam forgot this line? */
